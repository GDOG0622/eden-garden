...
async function callStatusAPI(char) {
    ...
    // 取聊天记录里绝对最后一条非系统消息（对应 ST 的 {{lastMessage}} 语义）
    // 无论是 AI 还是用户，都以最新那条为准
    const lastMsg = [...chat].reverse().find(m => !m.is_system && m.mes);
    const rawMes = lastMsg?.mes || '';
    // 优先提取 <content> 标签内内容（去除思维链等）
    const contentMatch = rawMes.match(/<content>([\s\S]*?)<\/content>/i);
    const lastMsgText = (contentMatch ? contentMatch[1] : rawMes).trim();
    const speaker = lastMsg
        ? (lastMsg.is_user ? (lastMsg.name || 'User') : (lastMsg.name || 'Assistant'))
        : '';
